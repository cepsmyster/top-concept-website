...
The section fills the screen. On the left, a column as tall as the cards holds the heading "Our Projects", the front project's number, name and description, and the See all projects link. On the right, the featured projects (`featured: true` in `projects`, `src/data.mjs`) sit as big square cards in a straight row, each with its type and name in the bottom-left corner. The section stays in place while the row slides right to left as you scroll, one project per step; when scrolling stops between two projects it glides on to the nearest one. The front project (next to the column) stays lit while the others dim, and the column's number, name and description change to match it. Once the row has run out of room, the last steps only move the highlight. Each description is the project's `text` in `src/data.mjs` (also shown on its project page). On phones the heading and description sit above the row. Visitors with reduced motion get a row that scrolls sideways. The markup is in the `index.html` page in `src/build.mjs`, the motion in "Our Projects" in `motion.js`, the styles under "Home: Our Projects" in `styles.css` (`--cs` is the card size).
